...
export class TodayComponent {
  ...
  private nba = resource({
    loader: () => this.espn.getScoreboard('basketball/nba', this.date).catch(() => EMPTY),
  });
  private nfl = resource({
    loader: () => this.espn.getScoreboard('football/nfl', this.date).catch(() => EMPTY),
  });
  private mlb = resource({
    loader: () => this.espn.getScoreboard('baseball/mlb', this.date).catch(() => EMPTY),
  });
  private nhl = resource({
    loader: () => this.espn.getScoreboard('hockey/nhl', this.date).catch(() => EMPTY),
  });
  ...
  teamData = computed(() =>
    MY_TEAMS.map((team) => {
      ...
      return {
        team,
        game: res ? this.espn.findTeamGame(res.value(), team.abbreviation) : null,
        isLoading: res?.isLoading() ?? false,
      };
    })
  );

  // ── Motorsport resources (60-day future window → always shows next race) ──
  private futureRange = (() => {
    const end = new Date();
  ...
  private f1 = resource({
    loader: () => this.espn.getScoreboard('racing/f1', this.futureRange).catch(() => EMPTY),
  });
  private nascar = resource({
    loader: () => this.espn.getScoreboard('racing/nascar-premier', this.futureRange).catch(() => EMPTY),
  });
  private indycar = resource({
    loader: () => this.espn.getScoreboard('racing/irl', this.futureRange).catch(() => EMPTY),
  });
  ...
  seriesData = computed(() =>
    MY_SERIES.map((s) => {
      ...
      return {
        series: s,
        race: this.espn.findNextRace(res.value()),
        isLoading: res.isLoading(),
      };
    })
  );

  // ── Cricket resource (TheSportsDB) ──────────────────────────────────
